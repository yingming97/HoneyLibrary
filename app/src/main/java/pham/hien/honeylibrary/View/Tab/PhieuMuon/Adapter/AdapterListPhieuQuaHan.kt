...
class AdapterListPhieuQuaHan(
    ...
    override fun onBindViewHolder(holder: ViewItemPhieuMuon, position: Int) {
        ...
        holder.tv_ma_phieu.text = phieuMuon.maPhieuMuon
        holder.tv_name.text = phieuMuon.tenDocGia
        holder.tv_thanh_tien.text = moneyFormatter(phieuMuon.tongTien)
        holder.tv_so_luong.text = phieuMuon.soLuong.toString()
        holder.tv_trang_thai.text = phieuMuon.trangThai
